...
set TARGETNAME=%~2
set PROJECTDIR=%~3
set NugetFolder=%PROJECTDIR%..\Nuget\
...
set NuGetCachePath=%NuGetCachePath%Template10.Services.NetworkAvailableService\
...
rem No need to append nuget.nuspec to %WorkingDirectory% target, otherwise 
rem 'specify a file name or directory name on the target' query will popup.
...
echo xcopy.exe %NuspecFile% %WorkingDirectory% /y 
if exist %NuspecFile% (
    xcopy.exe %NuspecFile% %WorkingDirectory% /y 
) else (
...
echo xcopy.exe %NupkegFile% "%NugetFolder%" /y >nul
xcopy.exe %NupkegFile% "%NugetFolder%" /y >nul
