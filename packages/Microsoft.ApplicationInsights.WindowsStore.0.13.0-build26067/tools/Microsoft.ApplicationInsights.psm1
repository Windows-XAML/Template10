...
# .SYNOPSIS
#   Removes Application Insights bootstrapping code from the given EnvDTE.Project.
function Remove-ApplicationInsights($project)
{
    if (!$project)
    {
        $project = Get-Project;
    ...
    Write-Verbose "Removing Application Insights bootstrapping code from $($project.FullName).";
    $entryPointClassName = Read-ApplicationEntryPoint $project;
    if ($entryPointClassName)
    {
        $entryPointClass = Find-CodeClass $project $entryPointClassName;
        Remove-ApplicationInsightsVariable $entryPointClass;
        Remove-ApplicationInsightsImport $entryPointClass;
    }
}
...
function Add-ApplicationInsightsImport($codeClass)
{
    ...
    if (!$import)
    {
        ...
        if (($codeClass.Language -eq [EnvDTE.CodeModelLanguageConstants]::vsCMLanguageCSharp) -and ($projectItem.DTE.Version -eq "12.0"))
        {
            ...
            $import = $projectItem.FileCodeModel.AddImport($applicationInsightsNamespace);
        }
        else
        {
            $import = $projectItem.FileCodeModel.AddImport($applicationInsightsNamespace, -1, $null);
        }
...
function Remove-ApplicationInsightsImport($codeClass)
{
    $import = Find-CodeImport $codeClass $applicationInsightsNamespace;
    if ($import)
    {
        $projectItem = $codeClass.ProjectItem;
        Write-Verbose "Removing $applicationInsightsNamespace import from $($projectItem.FileNames(1)).";
        $projectItem.FileCodeModel.Remove($import);
    }
}
...
function Add-ApplicationInsightsVariable($codeClass)
{
    ...
    {
        Write-Verbose "Adding the $telemetryClientVariableName static variable to the $($codeClass.FullName) class in $($codeClass.ProjectItem.FileNames(1)).";
        ...
        $telemetryClientVariable.InitExpression = "new TelemetryClient()";
        ...
        $telemetryClientVariable.DocComment = $docComment;
...
function Remove-ApplicationInsightsVariable($codeClass)
{
    $telemetryClientVariable = Find-CodeVariable $codeClass $telemetryClientVariableName;
    if ($telemetryClientVariable)
    {
        Write-Verbose "Removing the $($telemetryClientVariable.Name) variable from the $($codeClass.FullName) class in $($codeClass.ProjectItem.FileNames(1)).";
        $codeClass.RemoveMember($telemetryClientVariable);
    }
}
...
function Get-CodeElement($codeElements)
{
    ...
    {
        Write-Output $child;
        Get-CodeElement $child.Members $isMatch;
    }
}
...
Export-ModuleMember -Function Remove-ApplicationInsights;
# SIG # Begin signature block
# MIIazwYJKoZIhvcNAQcCoIIawDCCGrwCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU3crSyzvX7+oBzL1JSiXf+Pi3
# KkugghWCMIIEwzCCA6ugAwIBAgITMwAAAEyh6E3MtHR7OwAAAAAATDANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFICV
# Y435ZIzFGwztXk6ntMJGVdLKMHAGCisGAQQBgjcCAQwxYjBgoEaARABNAGkAYwBy
...
# AQUABIIBAGFMnqprburqoF//a7Vy0EgyZoLhwng88q7bieOVOTOk6HMBGVnzoHJp
# xZEXrArQW2eDDByclJ6+x8OmWt3Bq+sVbs8WFhneZ4hfPBXhtrcgkN1aX4mhfy1X
# UUwBPJbTu1PaLBBwGNiS/XEsgo2Y/x6uY8aE6qtCfM4bJtNnCqRXTiQLmARnAmpX
# SjhVVNiDJB5Pr/tFHK5ka7OipDV3vhEf+PTicJ4Ba1DnKLX1jgTNeeNj+dkmV3UY
# htVutGTF9fEdEtv5xjHM8TvRvXFbn1UFFFuOG5ZmSbfVJRm6kQdek8TyKpHFUrEC
# Zw4fSkCMOsAXN8w1vW6sDrI9E0FMoF2hggIoMIICJAYJKoZIhvcNAQkGMYICFTCC
...
# MB8GA1UEAxMYTWljcm9zb2Z0IFRpbWUtU3RhbXAgUENBAhMzAAAATKHoTcy0dHs7
# AAAAAABMMAkGBSsOAwIaBQCgXTAYBgkqhkiG9w0BCQMxCwYJKoZIhvcNAQcBMBwG
# CSqGSIb3DQEJBTEPFw0xNTAyMDIyMDM2MTlaMCMGCSqGSIb3DQEJBDEWBBSPxSB+
# 2kkqPOrv+FqWJ9jTUx+LozANBgkqhkiG9w0BAQUFAASCAQBZa6p/DfZlrBL3DWX3
# wlgZvlACCf2rS6er04dUCl/j8NRnMoG5HlbTgs7xy+pKvSzM1OEMlaZ6qaNqjsrw
# 40raGvu9Gte3VrHRZNCiLsU3GxmcfFqvrEJ3z6WTCA5MmqLdJzwxSa3UfEMENNeG
# 2WmFqjUoBNZUT6C4rda9CQ9p1Vz/+ZIUjUUl55NA9Bm2C2k9hU0I8bgxETbgk/k0
# f/UKa/X6XkjLDo+AitQMA6TLjpZU65JGswvOQNApsoJLXqDVSCbscPfbAcZY6Xlb
# VIKudxxtmjVy1V7QXZ99duSfkQCrKUZoOZ32mYwnWVbYjyFZz++BsW6aTcCmDu5W
# OKGp
