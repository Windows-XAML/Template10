	...
	public sealed partial class Shell : Page, INotifyPropertyChanged
	{
		...
		public static void SetRequestedTheme(ApplicationTheme theme)
		{
			...
			WindowWrapper.Current().Dispatcher.Dispatch(() =>
			{
				...
				Instance.MyHamburgerMenu.NavButtonPressedForeground = NavButtonPressedForegroundBrush;
				Instance.MyHamburgerMenu.NavButtonPressedBackground = NavButtonPressedBackgroundBrush;

				Instance.MyHamburgerMenu.NavButtonHoverForeground = NavButtonHoverForegroundBrush;
				...
				Instance.MyHamburgerMenu.NavButtonCheckedIndicatorBrush = NavButtonCheckedIndicatorBrush;
				Instance.MyHamburgerMenu.SecondarySeparator = SecondarySeparatorBrush;

				foreach (var el in Instance.MyHamburgerMenu.LoadedNavigationButtons)
				{
					if (el.HamburgerButtonInfo.ButtonType != HamburgerButtonInfo.ButtonTypes.Toggle) continue;
					ToggleButton btn = el.FrameworkElement as ToggleButton;
				...
				/*
				 // Okay, I can do without LoadedNavigationButtons, but LoadedNavigationButtons seems more elegant

				List<HamburgerButtonInfo> NavButtons = Instance.MyHamburgerMenu.PrimaryButtons.ToList();
				NavButtons.InsertRange(NavButtons.Count, Instance.MyHamburgerMenu.SecondaryButtons.ToList());
		...
		private static void ParseStyleforThemes(ApplicationTheme theme)
		{
			...
			{
				// hard-wired defaults below will take care of this!
			}
