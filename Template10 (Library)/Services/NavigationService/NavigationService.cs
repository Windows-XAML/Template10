...
{
    public class Nav : INavigationService
    {
        Services.SerializationService.ISerializationService serializationService;
        public Nav(Services.SerializationService.ISerializationService serializationService)
        {
        ...
        static void DebugWrite(string text = null, Services.LoggingService.Severities severity = Services.LoggingService.Severities.Template10, [CallerMemberName]string caller = null) =>
            LoggingService.LoggingService.WriteLine(text, severity, caller: $"{nameof(NavigationService)}.{caller}");

        #endregion

        List<Action<NavigatingCancelEventArgs>> NavigatingHandlers = new List<Action<NavigatingCancelEventArgs>>();
        List<Action<NavigationEventArgs>> NavigatedHandlers = new List<Action<NavigationEventArgs>>();
        CachingService _CachingService = new CachingService();
        public Nav(Frame frame)
        {
            Frame = frame;
            frame.Navigating += (s, e) => { NavigatingHandlers.ForEach(x => x(e)); };
            frame.Navigated += (s, e) => { NavigatedHandlers.ForEach(x => x(e)); };
        }

        public Frame Frame { get; private set; }
        public bool CanGoBack => Frame.CanGoBack;
        public bool CanGoForward => Frame.CanGoForward;
        public object CurrentPageParam{ get; private set; }
        ...
        public FrameFacade FrameFacade
        {
            get
            {
        ...
        public void ClearCache(bool removeCachedPagesInBackStack = false) => _CachingService.Clear(Frame, removeCachedPagesInBackStack);

        public async void GoBack() => await GoBackAsync();
        public Task<bool> GoBackAsync()
        {
            throw new NotImplementedException();
        }

        public async void GoForward() => await GoForwardAsync(); 
        public Task<bool> GoForwardAsync()
        {
            throw new NotImplementedException();
        }

        public async void Navigate(Type page, object parameter = null, NavigationTransitionInfo infoOverride = null) => await NavigateAsync(page, parameter, infoOverride);
        public Task<bool> NavigateAsync(Type page, object parameter = null, NavigationTransitionInfo infoOverride = null)
        {
            throw new NotImplementedException();
        }

        public async void Navigate<T>(T key, object parameter = null, NavigationTransitionInfo infoOverride = null) where T : struct, IConvertible => await NavigateAsync(key, parameter, infoOverride);
        ...
        public async void Open(Type page, object parameter = null, string title = null, ViewSizePreference size = ViewSizePreference.UseHalf) => await OpenAsync(page, parameter, title, size);
        public Task OpenAsync(Type page, object parameter = null, string title = null, ViewSizePreference size = ViewSizePreference.UseHalf)
        {
            throw new NotImplementedException();
        }
...
public partial class NavigationService : INavigationService
{
    ...
    public void ClearHistory()
    {
        FrameFacadeInternal.Frame.BackStack.Clear();
        FrameFacadeInternal.Frame.ForwardStack.Clear();
    }
